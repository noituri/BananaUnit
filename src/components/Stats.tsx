...
export default function Stats({ banana, objects }: AnalizeData) {
  if (objects.length === 0) {
    return (
      <h3 className="stats stats-error">
        Unfortunately, no objects were found :(
      </h3>
    );
  }
  ...
  const objectsLabels = objects.map((o) => <h3 key={o.label}>{o.label}</h3>);
  ...
  return (
    <div className="stats">
      ...
      <div className="stats-container">
        <div className="stats-labels secondary-font">{objectsLabels}</div>
        <div className="stats-areas">{objectsRanking}</div>
      </div>
    </div>
  );
}
